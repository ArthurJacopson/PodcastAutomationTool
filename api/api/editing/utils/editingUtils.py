...
def cut_video_with_smoothing(input_file, start_time, end_time, output_file, fade_duration=1):
    """
    ...
    audio = (input_stream.audio
             ...
             .filter('afade', type='out', start_time=end_time - start_time - fade_duration, duration=fade_duration))
    ffmpeg.output(video, audio, output_file).run(overwrite_output=True)

def render_video(input_file, output_file, codec_video='libx264', codec_audio='aac'):
    """
    ...
    """
    ffmpeg.input(input_file).output(output_file, vcodec=codec_video, acodec=codec_audio).run(overwrite_output=True)
...
def separate_audio_video(input_file, output_video_file, output_audio_file):
    ...
    """
    ffmpeg.input(input_file).output(output_video_file, an=None).run(overwrite_output=True)
    ffmpeg.input(input_file).output(output_audio_file, vn=None).run(overwrite_output=True)
...
def silence_part_of_video(input_file, start_time, end_time, output_file):
    ...
    video = input_stream.video
    audio = input_stream.audio.filter('volume', 'enable=between(t,{},{})'.format(start_time, end_time), volume=0)
    ffmpeg.output(video, audio, output_file).run(overwrite_output=True)
...
def add_audio_to_video(video_file, audio_file, output_file):
    ...
    ffmpeg.output(input_video.video, input_audio.audio, output_file, codec='copy').run(overwrite_output=True)
